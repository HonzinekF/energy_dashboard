...
function sanitizeData(data: Point[]) {
  ...
  const sanitized = data
    .filter((point) => {
      ...
      if (ts > maxFuture || ts < maxPast) return false;
      ...
      if (!base.every((v) => typeof v === "number" && Number.isFinite(v))) return false;
      if (!extra.every((v) => typeof v === "number" && Number.isFinite(v))) return false;
      return true;
    })
