...
export async function fetchSpotPrices(date?: string): Promise<SpotPricePayload | null> {
  const electricityData = await fetchFromElectricityPriceApi(date);
  if (electricityData) {
    return cachePayload(electricityData);
  }

  const spotovaData = await fetchFromSpotovaApi();
  if (spotovaData) {
    return cachePayload(spotovaData);
  }

  const entsoeData = await fetchFromEntsoeApi(date);
  if (entsoeData) {
    return cachePayload(entsoeData);
  }
...
async function fetchFromSpotovaApi(): Promise<SpotPricePayload | null> {
  try {
    ...
    const hourly = hours.map<SpotPricePoint>((entry, idx) => {
      const base = new Date();
      base.setHours(entry.hour, 0, 0, 0);
      base.setDate(base.getDate() + (idx >= 24 ? 1 : 0));
...
async function fetchFromEntsoeApi(date?: string): Promise<SpotPricePayload | null> {
  ...
  const pointsSource = firstSeries?.Period?.Point;
...
function cachePayload(payload: SpotPricePayload) {
