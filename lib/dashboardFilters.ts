...
export const DATA_SOURCES = [
  ...
  { value: "solax", label: "SolaX fallback" },
] as const;
...
export type DashboardSourceParam = (typeof DATA_SOURCES)[number]["value"];
export type DashboardSource = DashboardSourceParam | "live" | "python" | "excel" | "import";
...
export function normalizeSource(value: string | string[] | undefined): DashboardSource {
  ...
  const maybeValue = Array.isArray(value) ? value[0] : value;
  return DATA_SOURCES.some((option) => option.value === maybeValue) ? (maybeValue as DashboardSource) : DEFAULT_FILTERS.source;
}
...
export function normalizeSystem(value: string | string[] | undefined) {
  if (!value) return DEFAULT_FILTERS.system ?? "default";
